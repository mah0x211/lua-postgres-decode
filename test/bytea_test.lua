...
function testcase.bytea_hex()
    ...
    err = assert.throws(decode.bytea)
...
function testcase.bytea_escape()
    ...
    for _, a in ipairs({
        ...
        local v, err = decode.bytea(a, true)
        assert.equal(v, a)
        assert.is_nil(err)
    end

    -- test that throws an error if argument is not string
    local err = assert.throws(decode.bytea)
    assert.match(err, 'string expected,')
end
